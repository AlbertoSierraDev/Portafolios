    ...
    <section className="space-y-8 text-white">
      <div className="flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <p className="mt-3 max-w-2xl text-sm leading-7 text-white/60">
            Revisa los mensajes recibidos desde el formulario de contacto del
            portafolio y gestiona las oportunidades laborales.
          </p>
        </div>
        ...
        <button
          ...
          className="inline-flex items-center justify-center gap-2 rounded-xl border border-cyan-300/25 bg-cyan-300/10 px-5 py-3 text-sm font-semibold uppercase tracking-[0.22em] text-cyan-300 transition hover:bg-cyan-300/15"
        >
      ...
      <div className="grid gap-5 md:grid-cols-3">
        ...
        <div className="rounded-2xl border border-fuchsia-300/15 bg-fuchsia-300/10 p-5">
          ...
          <p className="mt-3 text-4xl font-black text-white">
            {messages.length - unreadMessages.length}
          </p>
        </div>
      </div>
      ...
      {messages.length === 0 ? (
        ...
        <div className="grid gap-5">
          {messages.map((message) => (
            <article
              key={message._id}
              className={`relative overflow-hidden rounded-[28px] border p-5 backdrop-blur-xl transition hover:-translate-y-1 md:p-6 ${
                message.read
                ...
                <div className="mb-5 flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
                  <div className="min-w-0">
                    <div className="mb-3 flex flex-wrap items-center gap-2">
                      {!message.read ? (
                        <span className="rounded-full border border-cyan-300/30 bg-cyan-300/10 px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.2em] text-cyan-300">
                          Nuevo
                        </span>
                      ) : (
